# Backup and Restore — GoldRush

## Overview

Daily encrypted backups of the entire `goldrush` Postgres database. Encryption uses a GPG key generated on the VPS during the first-time setup; the public key fingerprint must be stored OUTSIDE the VPS (1Password / hardware token) so it can be authenticated on a new VPS in case of disaster.

## Schedule

- **Daily** at 03:00 UTC, written to `/opt/goldrush/backups/daily/`. Retention: 30 days.
- **Monthly** on the 1st of each month, copied to `/opt/goldrush/backups/monthly/`. Retention: ~12 months.
- **Optional offsite** rsync to a Storage Box if `/root/.ssh/storagebox_key` and `STORAGEBOX_HOST` are configured.

The cron entry lives at `/etc/cron.d/goldrush-backup` and is installed by:
...
# expected: a fresh goldrush-<timestamp>.dump.gpg
```

## Restore — production (DESTRUCTIVE)

> This DROPS the entire `goldrush` database and replaces it with the contents of the chosen archive.

```bash
ssh -i ~/.ssh/id_ed25519 root@91.98.234.106
ls /opt/goldrush/backups/daily/    # find the archive you want
/opt/goldrush/repo/ops/scripts/restore.sh \
    /opt/goldrush/backups/daily/goldrush-2026-05-01T03-00-00Z.dump.gpg
...
2. Stops the D/W bot.
3. Drops and recreates the goldrush database.
...
```bash
ssh -i ~/.ssh/id_ed25519 root@91.98.234.106
RESTORE_DB=goldrush_drill \
    /opt/goldrush/repo/ops/scripts/restore.sh \
    /opt/goldrush/backups/daily/goldrush-2026-05-01T03-00-00Z.dump.gpg

# verify some rows are present
docker compose -f /opt/goldrush/repo/ops/docker/compose.yml exec postgres \
    psql -U goldrush_admin -d goldrush_drill -c "
    SELECT COUNT(*) FROM core.users;
    SELECT COUNT(*) FROM core.audit_log;"

# clean up
docker compose -f /opt/goldrush/repo/ops/docker/compose.yml exec postgres \
    psql -U goldrush_admin -d postgres -c "DROP DATABASE goldrush_drill;"
```
...
3. The public key was exported to `/opt/goldrush/secrets/backup-gpg-public.asc` after first setup; download a copy to your local machine for safekeeping (it doesn't need to be secret).
