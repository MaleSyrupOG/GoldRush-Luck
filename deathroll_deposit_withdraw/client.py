"""Discord client subclass for the DeathRoll Deposit/Withdraw bot.
...
- ``setup_hook`` (called by discord.py before ``on_ready``) opens
  the pool with the ``deathroll_dw`` Postgres role.
...
from deathroll_core.db import create_pool
from deathroll_core.ratelimit import FixedWindowLimiter

from deathroll_deposit_withdraw.cashiers.live_updater import OnlineCashiersUpdater
from deathroll_deposit_withdraw.metrics import start_metrics_server
from deathroll_deposit_withdraw.welcome import reconcile_welcome_embeds
from deathroll_deposit_withdraw.workers.audit_chain_verifier import (
    AuditChainVerifierWorker,
)
from deathroll_deposit_withdraw.workers.cashier_idle import CashierIdleWorker
from deathroll_deposit_withdraw.workers.claim_idle import ClaimIdleWorker
from deathroll_deposit_withdraw.workers.metrics_refresher import (
    MetricsRefresherWorker,
)
from deathroll_deposit_withdraw.workers.stats_aggregator import StatsAggregatorWorker
from deathroll_deposit_withdraw.workers.ticket_timeout import TicketTimeoutWorker
...
class DwBot(commands.Bot):
    ...
    async def on_ready(self) -> None:
        ...
        if self.pool is not None:
            ...
            # Story 8.3: spin up the cashier-idle worker (auto-offline
            # cashiers idle >1h every 5 min).
            try:
                if self._cashier_idle_worker is None:
                    self._cashier_idle_worker = CashierIdleWorker(pool=self.pool)
                    self._cashier_idle_worker.start()
                    self._log.info("cashier_idle_worker_started")
            except Exception as e:
