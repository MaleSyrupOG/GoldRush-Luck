-- =============================================================================
-- DeathRoll — schemas and per-role grant matrix.
--
...
GRANT USAGE ON SCHEMA core, fairness, luck     TO deathroll_luck;
GRANT USAGE ON SCHEMA core, fairness, dw       TO deathroll_dw;
GRANT USAGE ON SCHEMA core, fairness, poker    TO deathroll_poker;
GRANT USAGE ON SCHEMA core, fairness, luck, dw, poker TO deathroll_readonly;

-- =============================================================================
-- 3. Default privileges for FUTURE tables (created by Alembic)
-- =============================================================================

-- deathroll_luck: full RW on luck and fairness; SELECT-only on core.* tables
-- (writes to core.balances and core.users are deliberately denied; only
-- deathroll_dw can mint/destroy balance outside SECURITY DEFINER functions).
ALTER DEFAULT PRIVILEGES IN SCHEMA luck
    GRANT SELECT, INSERT, UPDATE ON TABLES TO deathroll_luck;
ALTER DEFAULT PRIVILEGES IN SCHEMA fairness
    GRANT SELECT, INSERT, UPDATE ON TABLES TO deathroll_luck;
ALTER DEFAULT PRIVILEGES IN SCHEMA luck
    GRANT USAGE, SELECT ON SEQUENCES TO deathroll_luck;
ALTER DEFAULT PRIVILEGES IN SCHEMA fairness
    GRANT USAGE, SELECT ON SEQUENCES TO deathroll_luck;
ALTER DEFAULT PRIVILEGES IN SCHEMA core
    GRANT SELECT ON TABLES TO deathroll_luck;
...
-- deathroll_dw: the economic frontier. Full RW on dw, fairness; targeted
...
-- core grants for deathroll_dw will be added per-table by the Alembic migration
...
-- deathroll_poker: future bot. Mirrors deathroll_luck on its own poker.* schema.
ALTER DEFAULT PRIVILEGES IN SCHEMA poker
    GRANT SELECT, INSERT, UPDATE ON TABLES TO deathroll_poker;
ALTER DEFAULT PRIVILEGES IN SCHEMA fairness
    GRANT SELECT, INSERT, UPDATE ON TABLES TO deathroll_poker;
ALTER DEFAULT PRIVILEGES IN SCHEMA poker
    GRANT USAGE, SELECT ON SEQUENCES TO deathroll_poker;
ALTER DEFAULT PRIVILEGES IN SCHEMA core
    GRANT SELECT ON TABLES TO deathroll_poker;

-- deathroll_readonly: SELECT everywhere. Used by Grafana, by Aleix locally
-- via SSH tunnel for debugging, and by audit_verify.py.
ALTER DEFAULT PRIVILEGES IN SCHEMA core
    GRANT SELECT ON TABLES TO deathroll_readonly;
ALTER DEFAULT PRIVILEGES IN SCHEMA fairness
    GRANT SELECT ON TABLES TO deathroll_readonly;
ALTER DEFAULT PRIVILEGES IN SCHEMA luck
    GRANT SELECT ON TABLES TO deathroll_readonly;
...
--     GRANT INSERT ON core.audit_log TO deathroll_luck, deathroll_dw, deathroll_poker;
