#!/usr/bin/env bash
# =============================================================================
# GoldRush — Postgres restore script.
#
# Restores a GPG-encrypted pg_dump archive into the goldrush database.
#
# WARNING: this DROPS and recreates the goldrush database. Make sure you have
# stopped the bots first. The script asks for confirmation before destroying
# data unless RESTORE_FORCE=1 is set.
#
# Usage (as root on the VPS):
#     ./restore.sh /opt/goldrush/backups/daily/goldrush-2026-05-01T03-00-00Z.dump.gpg
#
...
#   3. (If restoring into the canonical 'goldrush' DB) DROP and CREATE the DB.
...
RESTORE_DB="${RESTORE_DB:-goldrush}"
...
if [ "${RESTORE_FORCE:-0}" != "1" ]; then
    ...
    echo "    ${ARCHIVE}"
    echo
    if [ "${RESTORE_DB}" = "goldrush" ]; then
        echo "*** This will DROP and recreate the production 'goldrush' database. ***"
...
docker compose --env-file "${ENV_FILE}" -f "${COMPOSE_FILE}" stop \
    goldrush-deposit-withdraw 2>/dev/null || true
# (When Luck resumes, add: docker compose stop goldrush-luck)

# 3. drop / recreate target DB
if [ "${RESTORE_DB}" = "goldrush" ]; then
    echo "[restore] dropping and recreating goldrush DB"
    docker compose --env-file "${ENV_FILE}" -f "${COMPOSE_FILE}" exec -T postgres \
        psql --username "${PG_ADMIN_USER}" --dbname postgres -c "DROP DATABASE IF EXISTS goldrush;"
    docker compose --env-file "${ENV_FILE}" -f "${COMPOSE_FILE}" exec -T postgres \
        psql --username "${PG_ADMIN_USER}" --dbname postgres \
        -c "CREATE DATABASE goldrush OWNER ${PG_ADMIN_USER};"
else
...
# 7. restart bots (only if we restored into goldrush)
if [ "${RESTORE_DB}" = "goldrush" ]; then
