#!/usr/bin/env bash
# =============================================================================
# GoldRush — VPS first-time setup script.
#
...
#     cd /tmp && curl -LO https://raw.githubusercontent.com/MaleSyrupOG/GoldRush-Luck/main/ops/scripts/vps_first_setup.sh
#     chmod +x vps_first_setup.sh
#     ./vps_first_setup.sh
#
# What it does:
#   1. Creates the `goldrush` system user with home /opt/goldrush.
#   2. Adds the user to the `docker` group.
#   3. Creates the directory layout under /opt/goldrush/ with secure perms.
#   4. Generates the shared secrets in /opt/goldrush/secrets/.env.shared
...
#   5. Stages a placeholder /opt/goldrush/secrets/.env.dw with PASTE_*
#      placeholders that Aleix must fill in manually with the Discord token.
#   6. Clones (or pulls) the repository into /opt/goldrush/repo.
...
# other than /opt/goldrush/secrets/, and uses umask 077 throughout.
# =============================================================================
...
REPO_URL="${REPO_URL:-https://github.com/MaleSyrupOG/GoldRush-Luck.git}"
REPO_BRANCH="${REPO_BRANCH:-main}"

echo "==================================================================="
echo " GoldRush VPS first-time setup"
echo " repo : ${REPO_URL}"
echo " branch: ${REPO_BRANCH}"
echo "==================================================================="

# ----- 1. Create goldrush user -----------------------------------------------
if id -u goldrush >/dev/null 2>&1; then
    echo "[1/8] user 'goldrush' already exists — skipping creation"
else
    echo "[1/8] creating user 'goldrush'"
    adduser --system --group --shell /bin/bash --home /opt/goldrush goldrush
fi

# Ensure docker group membership
if id -nG goldrush | tr ' ' '\n' | grep -qx docker; then
    echo "      already in docker group"
else
    usermod -aG docker goldrush
    echo "      added to docker group"
fi

# ----- 2. Directory layout ---------------------------------------------------
echo "[2/8] creating directory layout under /opt/goldrush/"
install -d -o goldrush -g goldrush -m 750 /opt/goldrush
install -d -o goldrush -g goldrush -m 700 /opt/goldrush/secrets
install -d -o goldrush -g goldrush -m 750 /opt/goldrush/backups
install -d -o goldrush -g goldrush -m 750 /opt/goldrush/backups/daily
install -d -o goldrush -g goldrush -m 750 /opt/goldrush/backups/monthly
install -d -o goldrush -g goldrush -m 750 /opt/goldrush/logs
install -d -o goldrush -g goldrush -m 750 /opt/goldrush/scripts

# ----- 3. Generate shared secrets if missing ---------------------------------
SHARED_ENV="/opt/goldrush/secrets/.env.shared"
if [ -f "${SHARED_ENV}" ]; then
    ...
    umask 077
    cat > "${SHARED_ENV}" <<EOF
# GoldRush shared secrets — generated $(date -u +%Y-%m-%dT%H:%M:%SZ)
...
EOF
    chown goldrush:goldrush "${SHARED_ENV}"
    chmod 600 "${SHARED_ENV}"
fi

# ----- 4. Stage .env.dw with placeholders if missing -------------------------
DW_ENV="/opt/goldrush/secrets/.env.dw"
if [ -f "${DW_ENV}" ]; then
    ...
    umask 077
    cat > "${DW_ENV}" <<'EOF'
# GoldRush Deposit/Withdraw — bot-specific secrets
...
LOG_FORMAT=json
EOF
    chown goldrush:goldrush "${DW_ENV}"
    chmod 600 "${DW_ENV}"
fi

# ----- 5. Clone or pull the repository ---------------------------------------
REPO_DIR="/opt/goldrush/repo"
if [ -d "${REPO_DIR}/.git" ]; then
    echo "[5/8] repo already cloned — pulling latest from ${REPO_BRANCH}"
    sudo -u goldrush git -C "${REPO_DIR}" fetch --quiet origin "${REPO_BRANCH}"
    sudo -u goldrush git -C "${REPO_DIR}" checkout --quiet "${REPO_BRANCH}"
    sudo -u goldrush git -C "${REPO_DIR}" reset --hard --quiet "origin/${REPO_BRANCH}"
else
    echo "[5/8] cloning ${REPO_URL} into ${REPO_DIR}"
    sudo -u goldrush git clone --branch "${REPO_BRANCH}" --quiet "${REPO_URL}" "${REPO_DIR}"
fi

# ----- 6. GPG key for backups (root-owned) -----------------------------------
GPG_RECIPIENT_NAME="GoldRush Backup"
...
PUBKEY_EXPORT="/opt/goldrush/secrets/backup-gpg-public.asc"
if [ ! -f "${PUBKEY_EXPORT}" ]; then
    gpg --export --armor "${GPG_RECIPIENT_NAME}" > "${PUBKEY_EXPORT}"
    chown goldrush:goldrush "${PUBKEY_EXPORT}"
    chmod 644 "${PUBKEY_EXPORT}"
fi

# ----- 7. Permissions audit ---------------------------------------------------
echo "[7/8] permissions audit"
ls -la /opt/goldrush/ | sed 's/^/      /'
echo
ls -la /opt/goldrush/secrets/ | sed 's/^/      /'

# ----- 8. Next-step instructions ---------------------------------------------
echo "==================================================================="
echo " [8/8] DONE. Next steps:"
echo
echo "  1. Edit /opt/goldrush/secrets/.env.dw and replace the PASTE_*"
echo "     placeholders with your real Discord bot token and your"
echo "     Discord server (guild) ID."
echo
echo "       sudo -u goldrush nano /opt/goldrush/secrets/.env.dw"
echo
echo "  2. Confirm placeholders are gone:"
echo
echo "       grep -c PASTE_ /opt/goldrush/secrets/.env.dw"
echo "       (expected output: 0)"
echo
echo "  3. Build and start the stack:"
echo
echo "       cd /opt/goldrush/repo"
echo "       sudo -u goldrush docker compose --env-file /opt/goldrush/secrets/.env.shared -f ops/docker/compose.yml up -d --build"
echo
echo "  4. Tail logs until you see 'placeholder process running':"
echo
echo "       sudo -u goldrush docker compose -f /opt/goldrush/repo/ops/docker/compose.yml logs -f goldrush-deposit-withdraw"
echo
