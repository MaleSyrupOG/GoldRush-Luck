"""asyncpg pool helpers for the GoldRush platform.

Every bot acquires its own pool against the shared Postgres database with
its own DB role (goldrush_luck, goldrush_dw, etc.). The pool factory below
...
the database level (ALTER DATABASE goldrush SET app.audit_chain_key = ...)
...
async def create_pool(
    ...
    *,
    ...
    """Create an asyncpg connection pool with the GoldRush defaults.
